...
const newComponent = `    function KaryaDisha() {
      ...
      const renderAiText = (text) => {
         ...
         return text.split('\\n').map((line, i) => {
            ...
            if (line.startsWith('##')) return <h3 key={i} style={{ fontSize: 15, fontWeight: 700, margin: "16px 0 8px", color: "#1F2937" }}>{line.replace(/#/g, '').trim()}</h3>;
            ...
            {loadingData ? (
               ...
               <div className="two-panel-3-2">
                 ...
                 <div className="section-block">
                   <div style={{ padding: 20, borderRadius: 8, border: "1px solid #E5E7EB", boxShadow: "0 1px 2px rgba(0,0,0,0.03)", background: "#FFFFFF" }}>
                     ...
                     <div style={{ position: "relative", marginBottom: 20 }}>
                       <select value={role} onChange={e => { setRole(e.target.value); setRes(null); }} style={{ width: "100%", padding: "8px 12px", background: "#FFFFFF", color: role ? "#1F2937" : "#6B7280", border: "1px solid #D1D5DB", borderRadius: 6, fontSize: 14, outline: "none", appearance: "none" }}>
                         ...
                         return (
                           <button key={s} onClick={() => toggleSkill(s)} 
                             style={{ 
                               ...
                               borderRadius: 6, padding: "4px 10px", fontSize: 13, fontWeight: active ? 500 : 400, 
                               cursor: "pointer", display: "flex", alignItems: "center", gap: 6, transition: "all .1s ease" 
                             }}>
                             {s}
                             {lvl === "Beginner" && <span style={{ fontSize: 10, background: "#FF4F1F", color: "#fff", padding: "1px 4px", borderRadius: 4, fontWeight: 600 }}>Beg</span>}
                             {lvl === "Intermediate" && <span style={{ fontSize: 10, background: "#FF4F1F", color: "#fff", padding: "1px 4px", borderRadius: 4, fontWeight: 600 }}>Int</span>}
                             {lvl === "Advanced" && <span style={{ fontSize: 10, background: "#FF4F1F", color: "#fff", padding: "1px 4px", borderRadius: 4, fontWeight: 600 }}>Adv</span>}
                           </button>
                         );
                       })}
                     </div>

                     <button onClick={analyze} disabled={!role || Object.keys(skills).length === 0} style={{ width: "100%", padding: "10px", fontSize: 14, fontWeight: 600, color: "#FFFFFF", background: (!role || Object.keys(skills).length === 0) ? "#F3F4F6" : "#FF4F1F", border: "none", borderRadius: 6, cursor: (!role || Object.keys(skills).length === 0) ? "not-allowed" : "pointer" }}>Analyze Readiness</button>
                   </div>
                 </div>
                 ...
                 <div className="section-block" style={{ gap: 16 }}>
                   {!res ? (
                     <div style={{ padding: "40px 20px", textAlign: "center", borderRadius: 8, border: "1px dashed #D1D5DB", background: "#FAFAFA" }}>
                     ...
                     <>
                       ...
                       <div style={{ padding: 20, borderRadius: 8, border: "1px solid #E5E7EB", boxShadow: "0 1px 2px rgba(0,0,0,0.03)", background: "#FFFFFF" }}>
                          ...
                          {res.strong.length > 0 && (
                             <div style={{ marginBottom: 12 }}>
                                <div style={{ fontSize: 12, color: "#4B5563", marginBottom: 6, fontWeight: 600 }}>Strong Skills</div>
                          ...
                          {res.developing.length > 0 && (
                             <div style={{ marginBottom: 12 }}>
                                <div style={{ fontSize: 12, color: "#4B5563", marginBottom: 6, fontWeight: 600 }}>Developing Skills</div>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          ...
                          {res.missingCore.length > 0 && (
                             <div style={{ marginBottom: 12 }}>
                                <div style={{ fontSize: 12, color: "#4B5563", marginBottom: 6, fontWeight: 600 }}>Missing Core Skills</div>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          ...
                          {res.bonus.length > 0 && (
                             <div style={{ marginBottom: 12 }}>
                                <div style={{ fontSize: 12, color: "#4B5563", marginBottom: 6, fontWeight: 600 }}>Bonus Skills</div>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                       ...
                       {/* 3. Priority to Learn */}
                       <div style={{ padding: 20, borderRadius: 8, border: "1px solid #E5E7EB", boxShadow: "0 1px 2px rgba(0,0,0,0.03)", background: "#FFFFFF" }}>
                          <div style={{ fontSize: 16, fontWeight: 600, color: "#1F2937", marginBottom: 12 }}>Priority Learning List</div>
                       ...
                       {/* 4. AI Insights */}
                       <div style={{ padding: 20, borderRadius: 8, border: "1px solid #E5E7EB", boxShadow: "0 1px 2px rgba(0,0,0,0.03)", background: "#FFFFFF" }}>
                          <div style={{ fontSize: 16, fontWeight: 600, color: "#1F2937", marginBottom: 6 }}>AI Career Advisor</div>
                          <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 16 }}>Get personalized AI advice based on your exact profile.</p>
                          
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                             <button onClick={() => callAi('gap-analysis')} disabled={aiResponse.loading} style={{ background: "#FFFFFF", color: "#1F2937", border: "1px solid #D1D5DB", fontSize: 13, padding: "6px 12px", borderRadius: 6, cursor: "pointer", fontWeight: 500 }}>Explain Skill Gaps</button>
                             <button onClick={() => callAi('roadmap')} disabled={aiResponse.loading} style={{ background: "#FFFFFF", color: "#1F2937", border: "1px solid #D1D5DB", fontSize: 13, padding: "6px 12px", borderRadius: 6, cursor: "pointer", fontWeight: 500 }}>Generate Roadmap</button>
                             <button onClick={() => document.querySelector('.ai-fab')?.click()} style={{ background: "#FFFFFF", color: "#1F2937", border: "1px solid #D1D5DB", fontSize: 13, padding: "6px 12px", borderRadius: 6, cursor: "pointer", fontWeight: 500 }}>Ask AI via Chat</button>
                          </div>

                          {(aiResponse.gap || aiResponse.roadmap || aiResponse.loading || aiResponse.error) && (
                             <div style={{ padding: 16, borderRadius: 6, background: "#F9FAFB", border: "1px solid #E5E7EB", marginTop: 12 }}>
                       ...
                       <div style={{ padding: 20, borderRadius: 8, border: "1px solid #E5E7EB", boxShadow: "0 1px 2px rgba(0,0,0,0.03)", background: "#FFFFFF" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                             ...
                             <button onClick={() => callAi('project-recommendations')} disabled={aiResponse.loading} style={{ background: "#FFFFFF", color: "#1F2937", border: "1px solid #D1D5DB", padding: "4px 8px", fontSize: 12, borderRadius: 4, cursor: "pointer", fontWeight: 500 }}>AI Recommend</button>
                          </div>
                          ...
                          {aiResponse.projects ? (
                             ...
                             <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                {(res.projs || []).map((p, i) => (
                                   <div key={i} style={{ border: "1px solid #E5E7EB", background: "#FAFAFA", borderRadius: 6, padding: "10px 12px", display: "flex", gap: 10, alignItems: "center" }}>
                       ...
                       <div style={{ padding: 20, borderRadius: 8, border: "1px solid #E5E7EB", boxShadow: "0 1px 2px rgba(0,0,0,0.03)", background: "#FFFFFF" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                             ...
                             <button onClick={findJobs} disabled={loadingJobs} style={{ background: "#FFFFFF", color: "#1F2937", border: "1px solid #D1D5DB", padding: "4px 8px", fontSize: 12, borderRadius: 4, cursor: "pointer", fontWeight: 500 }}>{loadingJobs ? "Matching..." : "Find Matches"}</button>
                          </div>
                          ...
                          {compareRoles.length > 0 && (
                             <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                {compareRoles.map(cr => {
                                   ...
                                   return (
                                      <div key={cr} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#FAFAFA", border: "1px solid #E5E7EB", padding: "10px 12px", borderRadius: 6 }}>
                                         <span style={{ fontSize: 13, color: "#374151", fontWeight: 500 }}>{cr}</span>
                       ...
                       {/* 8. Progress */}
                       <div style={{ padding: 20, borderRadius: 8, border: "1px solid #E5E7EB", boxShadow: "0 1px 2px rgba(0,0,0,0.03)", background: "#FFFFFF" }}>
                          <div style={{ fontSize: 16, fontWeight: 600, color: "#1F2937", marginBottom: 12 }}>Career Progress</div>
                          {getProgressHistory().length > 0 ? (
                             <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                {getProgressHistory().slice().reverse().map((h, i) => (
                                   <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: i === getProgressHistory().length - 1 ? "none" : "1px solid #E5E7EB", paddingBottom: i === getProgressHistory().length - 1 ? 0 : 8 }}>
